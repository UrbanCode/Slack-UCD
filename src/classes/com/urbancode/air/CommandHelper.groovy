...
public class CommandHelper {
    ...
    static private Collection<String> specialChars;
    static {
    ...
    public CommandHelper(dir, customSpecialChars) {
        ...
        // String customChars = new String(customSpecialChars);
        // Set<String> chars = new LinkedHashSet<String>();
        // Collections.addAll(chars, customChars.split("(?!^)"));
        // specialChars = Collections.unmodifiableCollection(chars);
    ...
    public int runCommandAsBash(def message, def command, Closure closure) {
        return runCommandAsBash(message, command, closure, null)
    }

    public int runCommandAsBash(def message, def command, Closure closure, String input) {
      
        command[0] = sanitizeExecutable(command[0])
        def commandLine = command.collect{addDisplayQuotes(it)}.join(' ');

        def bashCommand = [];
        bashCommand << "/bin/bash"
        bashCommand << "-c"
        bashCommand << commandLine

        pb.command(bashCommand as String[])
    ...
    private boolean requiresDisplayQuotes(String value) {
        ...
        if (value == "" || specialChars.find{value.contains(it)}) {
            ...
            println "special char found in argument"

            if (value != "" && (value[0] == "1" || value[0] == "2" || value[0] == "\$")) {
                return false;
            }
            else {
                return true;
